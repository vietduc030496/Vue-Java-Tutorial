...
// onMounted(() => console.log(idInput.value))
...
async function getDataById() {
    ...
    console.log(get_id.value.value);

    const id = get_id.value.value;
      ...
      if (id) {
        try {
          ...
          getResult.value.length = 0;
          getResult.value.push(res.data);
        } catch (err) {
          getResult = fortmatResponse(err.response?.data) || err;
        }
      }
}
